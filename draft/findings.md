...
## ALFWorld 环境中的现有 Skill Bank

### 总览
| 类别 | 数量 | 说明 |
|------|------|------|
| 通用技能（General） | 12 | gen_001 ~ gen_012，跨任务通用原则 |
| pick_and_place | 5 | pic_001 ~ pic_005 |
| look_at_obj_in_light | 5 | loo_001 ~ loo_005 |
| clean | 6 | cle_001 ~ cle_006 |
| heat | 5 | hea_001 ~ hea_005 |
| cool | 5 | coo_001 ~ coo_005 |
| examine | 6 | exa_001 ~ exa_006 |
| 常见错误（Mistakes） | 11 | err_001 ~ err_011 |
| **总计** | **55** | |

### 动态生成的 Skills 命名规则
- 前缀 `dyn_NNN`（如 `dyn_001`、`dyn_002`）
- 默认加入 `general_skills` 类别
- 在 template 模式下始终被包含（不受 top_k 限制）
